...
from flame.FLAME import FLAME, TexturedFLAME
...
def main(arg):
    ...
    flame_conf.texture_path = '/home/lichnost/programming/work/ml/head/FLAME/model/FLAME_texture.npz'
    ...
    # )
    #
    # lights = PointLights(location=[[0.0, 0.0, -1.0]], device=devices[0])
    #
    # renderer = MeshRenderer(
    #     rasterizer=MeshRasterizer(cameras=camera, raster_settings=raster_settings),
    ...
    landmark_trn = flame.transform_points(landmarks)
    landmark_trn[:, :, 0] *= -1
    landmark_trn[:, :, 1] *= -1
    ...
    landmark_trn = landmark_trn[0].detach().cpu().numpy()
